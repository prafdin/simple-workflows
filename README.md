...
Start from `deploy/overlays/sample-with-mtls` instead of `deploy/overlays/sample` — it wires an initContainer that builds a client certificate PEM from a Secret (the app's distroless image has no shell to do this itself) and points `MONGO_URI` at it via `tlsCAFile`/`tlsCertificateKeyFile`. Fill in `mongo-ca-secret.yaml` and `mongo-client-secret.yaml` with your MongoDB's actual CA certificate and client cert/key — e.g. from PSMDB's `<cluster-name>-ssl` Secret, which holds `ca.crt`, `tls.crt`, `tls.key` (Secret volumes can't cross namespaces, so these values have to be copied into your own namespace regardless of where the source MongoDB generates them).
